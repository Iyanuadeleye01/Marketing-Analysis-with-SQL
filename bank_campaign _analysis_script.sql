...
CREATE OR REPLACE VIEW job_subscription_rate AS
SELECT job, 
...
CREATE OR REPLACE VIEW age_subscription_rate AS
SELECT
	CASE
...
CREATE OR REPLACE VIEW marital_subscription_rate AS
SELECT marital,
				COUNT(*) AS total,
...
CREATE OR REPLACE VIEW education_subscription_rate AS
SELECT education,
	COUNT(*) AS total,
    SUM(CASE WHEN y = 'yes' THEN 1 ELSE 0 END) AS subscribed,
...
CREATE OR REPLACE VIEW housing_subscription_rate AS
SELECT housing,
		COUNT(*) AS total,
        SUM(CASE WHEN y = 'yes' THEN 1 ELSE 0 END) AS subscribed,
...
CREATE OR REPLACE VIEW month_subscription_rate AS
SELECT `month`,
		COUNT(*) AS total,
        SUM(CASE WHEN y = 'yes' THEN 1 ELSE 0 END) AS subscribed,
        SUM(CASE WHEN y = 'no' THEN 1 ELSE 0 END) AS not_subscribed,
...
CREATE OR REPLACE VIEW poutcome_subscription_rate AS
SELECT poutcome,
       COUNT(*) AS total,
       SUM(CASE WHEN y = 'yes' THEN 1 ELSE 0 END) AS subscribed,
       SUM(CASE WHEN y = 'no' THEN 1 ELSE 0 END) AS not_subscribed,
...
CREATE OR REPLACE VIEW duration_subscription_rate AS
SELECT 
...
CREATE OR REPLACE VIEW overall_campaign_success AS
SELECT y AS subscription_status,
		COUNT(*) AS total_count
FROM campaign_data
GROUP BY y;

-- Does account balance influence subscription behaviour?
CREATE OR REPLACE VIEW balance_subscription_effect AS
SELECT y as subscription_status,
